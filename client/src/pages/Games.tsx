...
import { Grid, List, Filter, Trophy, Heart, Sparkles, Flower2, PlayCircle } from "lucide-react";

type MiniId = "catcher" | "match" | "garden";
const MINI_GAMES: { id: MiniId; title: string; blurb: string; emoji: string; gradient: string; bg: string }[] = [
  { id: "catcher", title: "Kindness Catcher", blurb: "Catch falling kind words", emoji: "💗", gradient: "from-rose-500 to-pink-600",
    bg: "from-rose-100 via-pink-100 to-orange-100 dark:from-rose-950/40 dark:via-pink-950/30 dark:to-slate-900" },
  { id: "match", title: "Emotion Memory", blurb: "Match the feelings", emoji: "🧠", gradient: "from-fuchsia-500 to-purple-600",
    bg: "from-fuchsia-100 via-purple-100 to-indigo-100 dark:from-fuchsia-950/40 dark:via-purple-950/30 dark:to-slate-900" },
  { id: "garden", title: "Peace Garden", blurb: "Grow a calm garden", emoji: "🌷", gradient: "from-emerald-500 to-teal-600",
...
export default function Games() {
  ...
  const [activeMini, setActiveMini] = useState<MiniId | null>(null);
  const { user } = useAuth();
  ...
  return (
    <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Quick Mini-Games — fast, animated, popup-launched */}
        <motion.section
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-fuchsia-500" /> Quick Mini-Games
              </h2>
              <p className="text-sm text-slate-600 dark:text-slate-400">Fast, animated games — perfect for a 60-second break.</p>
            </div>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {MINI_GAMES.map((g, i) => {
              const Icon = g.id === "catcher" ? Heart : g.id === "match" ? Sparkles : Flower2;
              return (
                <motion.button
                  key={g.id}
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setActiveMini(g.id)}
                  className="text-left"
                >
                  <Card className={`relative overflow-hidden p-0 h-full bg-gradient-to-br ${g.bg} border-2 border-white/50 dark:border-slate-700/50 shadow-lg`}>
                    <div className="absolute -top-3 -right-1 text-7xl opacity-20 select-none rotate-12">{g.emoji}</div>
                    <div className="relative p-5">
                      <div className={`inline-flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br ${g.gradient} text-white shadow-lg mb-3`}>
                        <Icon className="w-5 h-5" />
                      </div>
                      <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">{g.title}</h3>
                      <p className="text-xs text-slate-700 dark:text-slate-300 mb-3">{g.blurb}</p>
                      <div className="inline-flex items-center gap-1 text-xs font-bold text-slate-800 dark:text-slate-100 bg-white/70 dark:bg-slate-800/70 px-2.5 py-1 rounded-full">
        ...
        {/* Mini-game popup */}
        {activeMini && (() => {
          const g = MINI_GAMES.find((m) => m.id === activeMini)!;
          return (
            <MiniGameShell open={!!activeMini} onClose={() => setActiveMini(null)} title={g.title} gradient={g.gradient}>
              {activeMini === "catcher" && <KindnessCatcher />}
              {activeMini === "match" && <EmotionMatch />}
              {activeMini === "garden" && <PeaceGarden />}
            </MiniGameShell>
          );
        })()}
      </div>
